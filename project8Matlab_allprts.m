...
mc.rpm_cont_index      = [0,200,400,600,800,1000,1200,1400,1600,1800,...
    ...
    7000,7200,7400,7600,7800,8000, 8200]; % rpm
mc.trq_cont_map 	    = [180.0	180.0	180.0	180.0	180.0...
    ...
    80.0	77.8	75.8	73.8	72.0    0.0] ; % (N*m)

% Max (peak) operation limits
mc.rpm_max_index 	    = mc.rpm_cont_index;
mc.trq_max_map      	=[300.0	300.0	300.0	300.0	300.0	300.0...
    300.0	300.0	300.0	300.0	300.0	300.0	300.0	300.0...
    300.0	300.0	300.0		282.4	266.7	252.6	240.0...
    228.6	218.2	208.7	200.0	192.0	184.6	177.8	171.4...
    165.5	160.0	154.8	150.0	145.5	141.2	137.1	133.3...
    129.7	126.3	123.1	120.0   0.0];

% Efficiency data, rows indexed by rpm and columns indexed by trq
mc.rpm_eff_index 		= linspace(0,8000,25);
mc.trq_eff_index 	    = linspace(0,300,12);
mc.eff_trq_map		    = [...
...
legend('2.36 * Baseline motor','Baseline motor')
xlabel('Time (Seconds)')
ylabel('Vehicle Velocity (Kmph)')
title('Vehicle Speed against Time')
hold off
